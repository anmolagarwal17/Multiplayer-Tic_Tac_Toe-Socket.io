...
const createDirectoryIfNotExists = require('./utils')
	...
	socket.on('socket.io-file::createFile', (fileInfo) => {
		...
		if (typeof options.uploadDir === 'string') {
			...
			if (!uploadTo) {
				return sendError(
					new Error(
						'Upload directory must be specified in multiple directories.'
					)
				);
				...
				return sendError(
					new Error(
						'Upload directory ' + uploadTo + ' is not exists.'
					)
				);
			}
			...
			return sendError(
				new Error(
					'Max Uploading File size must be under ' +
						this.maxFileSize +
						' byte(s).'
				)
			);
		...
		socket.on(`socket.io-file::stream::${id}`, (chunk) => {
			...
			function write() {
				...
				if (
					...
					return sendError(
						new Error(
							`Uploading file size exceeded max file size ${self.maxFileSize} byte(s).`
						)
					);
				}
